...
ssh "root@$pve_host" "pct create $vmid /var/lib/vz/template/cache/$hostname-$template_filename --hostname $hostname --memory $memory --cores $cores --rootfs local-zfs:$disk_size --unprivileged 1 --features nesting=1 --onboot 1 --net0 $net_config"
echo
