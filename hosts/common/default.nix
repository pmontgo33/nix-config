...
{ config, lib, pkgs, inputs, outputs, ... }: {
  ...
  # Option for host-specific justfile
  options.hostSpecificJustfile = lib.mkOption {
    type = lib.types.nullOr lib.types.path;
    default = null;
    description = "Path to host-specific justfile to append to the common justfile";
  };
  ...
  config = {
    ...
    system.activationScripts.copyJustfile =
      if config.hostSpecificJustfile != null then ''
        cat ${./.dotfiles/justfile} ${config.hostSpecificJustfile} > /root/justfile
